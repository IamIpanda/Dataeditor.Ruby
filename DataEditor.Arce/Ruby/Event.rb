...
class DataEditor::Control::Event::DataModel::CommandType
	def initialize(code = -1, follow = -1, command = "Untitled",name = "Unknown", text = nil, parameters = "", window = nil, with = nil, ends = -1)
		self.Code = code
		self.Follow = follow
...
class DataEditor::Control::Event::DataModel::CommandGroup
	def AddCommands(source, indecies)
		for index in indecies
			target = source[index]
			AddCommand(target) if target != nil
		end
	end
end
...
class Event_Help
	class <<self
		...
		def variable_or_values(type, *ints)
			return type == 0 ? ints.join(", ") : ("变量".encode + ints.collect{ |i| sprintf("[%04d]",i) }.join("") )
		end
		def value(id, data)
