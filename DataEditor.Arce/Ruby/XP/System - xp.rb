...
Builder.Add(:tab , { :text => "系统" }) do
	group = Builder.Add(:group) do
		...
		Builder.Add(:group, {:text => "系统 图形 \/ BGM \/ ME \/ SE"}) do
			Builder.Add(:oldimage , {:actual => :windowskin_name , :text => "窗口外观图形" ,:path => "Graphics/WindowSkins/"})
			Builder.Add(:oldimage , {:actual => :title_name , :text => "标题画面图形" ,:path => "Graphics/Titles"})
			Builder.Add(:oldimage , {:actual => :gameover_name , :text => "游戏结束图形" ,:path => "Graphics/GameOvers"})
			Builder.Add(:oldimage , {:actual => :battle_transition , :text => "战斗渐变图形" ,:path => "Graphics/Transitions"})
